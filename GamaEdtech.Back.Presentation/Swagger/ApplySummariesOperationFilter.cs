...
namespace GamaEdtech.Back.Application.Swagger
{
    public class ApplySummariesOperationFilter : IOperationFilter
    {
        public void Apply(OpenApiOperation operation, OperationFilterContext context)
        {
            ...
            else if (IsActionName("Post", "Create", "Register"))
            {
                if (!operation.Summary.HasValue())
            ...
            else if (IsActionName("DeleteAll", "RemoveAll"))
            {
                if (!operation.Summary.HasValue())
                    operation.Summary = $"Deletes All {singularizeName}";
            }

            else if (IsActionName("DeleteToLie", "RemoveToLie", "RemoveToLieAsync", "DeleteToLieAsync", "DeleteToLieOrUpsideDown"))
            {
                if (!operation.Summary.HasValue())
                    operation.Summary = $"Delete to lie {singularizeName}";
            }
